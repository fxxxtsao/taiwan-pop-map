// meiko_stage + meiko_geocoded → web/meiko.json
// 縣市/區以地址字串為準，做落區驗證：座標不在該區界內 → 退回鄉鎮中心（src=town）。
...
const stage = JSON.parse(fs.readFileSync(path.join(dataDir, 'meiko_stage.json'), 'utf8'));
const geo = JSON.parse(fs.readFileSync(path.join(dataDir, 'meiko_geocoded.json'), 'utf8'));
...
for (const s of stage) {
  ...
  const loc = parseLoc(s.addr) || { county: s.county, town: null, f: null };
  ...
  cnt[src] = (cnt[src] || 0) + 1;
  rows.push([s.name, lat, lon, loc.county || s.county, loc.town || '', src]);
}
const out = { updated: new Date().toISOString().slice(0, 10), source: '明光義塾 教室清單', rows };
fs.writeFileSync(path.join(__dirname, '..', 'web', 'meiko.json'), JSON.stringify(out));
console.log(`exported ${rows.length}（路名 ${cnt.road || 0}, 鄉鎮中心 ${cnt.town || 0}）→ web/meiko.json`);
