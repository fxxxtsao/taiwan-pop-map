// 台灣拓人（TACT）地址地理編碼（Nominatim 路名級，免金鑰免費，共用 geocode_cache.json，可中斷續跑）
// 鍵：s.key（county+name）。近似座標供畫面測試；日後要門牌級再改走 Google。
const fs = require('fs');
...
(async () => {
  const outPath = path.join(dataDir, 'tact_geocoded.json');
  const results = fs.existsSync(outPath) ? JSON.parse(fs.readFileSync(outPath, 'utf8')) : {};
  console.log('to geocode:', stage.length);
  let n = 0, saved = 0;
  for (const s of stage) {
    n++;
    if (results[s.key] && results[s.key].src !== 'none') continue;
    const v = await query(roadOnly(s.addr));
    if (v) results[s.key] = { lat: v.lat, lon: v.lon, src: 'road' };
    else results[s.key] = { src: 'none' };
    if (++saved % 25 === 0) {
