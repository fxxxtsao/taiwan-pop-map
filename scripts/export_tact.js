// tact_stage + tact_geocoded → web/tact.json
...
const stage = JSON.parse(fs.readFileSync(path.join(dataDir, 'tact_stage.json'), 'utf8'));
const geo = JSON.parse(fs.readFileSync(path.join(dataDir, 'tact_geocoded.json'), 'utf8'));
...
for (const s of stage) {
  ...
  if (loc.f && !geoContains(loc.f, [lon, lat])) {
    const [clon, clat] = geoCentroid(loc.f);
...
const out = { updated: new Date().toISOString().slice(0, 10), source: '台灣拓人個別指導 教室清單', rows };
fs.writeFileSync(path.join(__dirname, '..', 'web', 'tact.json'), JSON.stringify(out));
console.log(`exported ${rows.length}（路名 ${cnt.road || 0}, 鄉鎮中心 ${cnt.town || 0}）→ web/tact.json`);
if (relocated.length) console.log(`跨區退回中心 ${relocated.length} 校`);
if (missed.length) console.log(`查無座標 ${missed.length} 校: ${missed.join('、')}`);
