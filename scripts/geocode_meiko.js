// 明光義塾地址地理編碼（Nominatim 路名級，免金鑰免費，共用 geocode_cache.json，可中斷續跑）
// 近似座標，供畫面測試；日後要門牌級再改走 Google（geocode_google 那套）。
const fs = require('fs');
...
(async () => {
  const outPath = path.join(dataDir, 'meiko_geocoded.json');
  const results = fs.existsSync(outPath) ? JSON.parse(fs.readFileSync(outPath, 'utf8')) : {};
  console.log('to geocode:', stage.length);
  let n = 0, saved = 0;
  for (const s of stage) {
    n++;
    if (results[s.name] && results[s.name].src !== 'none') continue;
    const v = await query(roadOnly(s.addr));
    if (v) results[s.name] = { lat: v.lat, lon: v.lon, src: 'road' };
    else results[s.name] = { src: 'none' };
    if (++saved % 25 === 0) {
